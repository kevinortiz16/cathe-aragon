    ...
    <div className="mx-auto max-w-6xl px-4 py-16">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
        <div>
          ...
          <p className="text-tatakoa-charcoal/60 max-w-xl">
      ...
      {(!items || items.length === 0) && (
        <p className="text-tatakoa-charcoal/50">Próximamente, casos de éxito con marcas.</p>
      )}
        ...
        {items?.map((item: PortfolioItem) => (
          ...
          >
            ...
            <div className="p-6">
              ...
              <p className="text-sm text-tatakoa-charcoal/60 mt-2">{item.description}</p>

              <div className="flex items-center justify-between mt-5 pt-5 border-t border-black/5">
                {item.metric_label && item.metric_value && (
                  <div>
                    <p className="text-2xl font-semibold text-tatakoa-sand">
                      {item.metric_value}
                    </p>
                    <p className="text-xs text-tatakoa-charcoal/50">{item.metric_label}</p>
                  </div>
                )}
                {item.external_link && (
                  <Link
      ...
      <div className="mt-16 text-center rounded-2xl bg-tatakoa-sand/5 py-12 px-6">
        <h2 className="text-2xl font-semibold mb-2">¿Buscas colaborar?</h2>
        <p className="text-tatakoa-charcoal/60 mb-6 max-w-md mx-auto">
